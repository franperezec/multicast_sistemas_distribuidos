...
class NetworkMonitor:
    ...
    def setup_socket(self):
        ...
        try:
            ...
            print_colored(f"✓ Monitor configurado en {MULTICAST_GROUP}:{PORT}", 'GREEN')
            return True
        ...
        while self.running:
            ...
            with self.lock:
                ...
                if time_diff > 0:
                    ...
                    message_diff = self.stats['total_messages'] - self.last_message_count
                    messages_per_second = message_diff / time_diff
                    self.message_history.append(messages_per_second)
                    
                    # Calcular bytes por segundo
                    byte_diff = self.stats['total_bytes'] - self.last_byte_count
    ...
    def display_stats(self):
        ...
        with self.lock:
            ...
            avg_msg_rate = sum(self.message_history) / len(self.message_history) if self.message_history else 0
            avg_bw_rate = sum(self.bandwidth_history) / len(self.bandwidth_history) if self.bandwidth_history else 0
            
            # Mostrar encabezado
            print(COLORS['CYAN'] + "="*70 + COLORS['RESET'])
            print(COLORS['CYAN'] + "   MONITOR DE RED MULTICAST - TIEMPO REAL" + COLORS['RESET'])
            print(COLORS['CYAN'] + "="*70 + COLORS['RESET'])
            ...
            if self.stats['messages_per_node']:
                ...
                for node, count in sorted_nodes:
                    ...
                    print(f"   {node}: {count} msgs ({percentage:.1f}%), {bytes_sent:,} bytes")
            ...
            if self.stats['message_types']:
                ...
                for msg_type, count in sorted(self.stats['message_types'].items(), 
                    ...
                    bar = "█" * bar_length
                    print(f"   {msg_type:<12}: {bar} {count} ({percentage:.1f}%)")
            ...
            print(f"{COLORS['WHITE']}Presiona Ctrl+C para detener el monitor{COLORS['RESET']}")
        ...
        with self.lock:
            try:
                with open(filename, 'w', encoding='utf-8') as f:
                    ...
                    f.write("-"*30 + "\n")
                    f.write(f"Duración del monitoreo: {int(time.time() - self.stats['start_time'])} segundos\n")
                    f.write(f"Total de mensajes: {self.stats['total_messages']}\n")
                    f.write(f"Total de bytes: {self.stats['total_bytes']}\n")
                    f.write(f"Nodos detectados: {len(self.stats['nodes_seen'])}\n")
                    ...
                    for node, count in sorted(self.stats['messages_per_node'].items()):
                        bytes_sent = self.stats['bytes_per_node'][node]
                        f.write(f"{node}: {count} mensajes, {bytes_sent} bytes\n")
    ...
    def start(self):
        ...
        threads = [
            ...
            threading.Thread(target=self.update_rates_thread, name="RateUpdater"),
    ...
    def stop(self):
        ...
        with self.lock:
            ...
            print_colored(f"   Total mensajes: {self.stats['total_messages']}", 'WHITE')
            print_colored(f"   Total bytes: {self.stats['total_bytes']:,}", 'WHITE')
            print_colored(f"   Nodos detectados: {len(self.stats['nodes_seen'])}", 'WHITE')
            
            if self.stats['total_messages'] > 0:
                avg_msg_size = self.stats['total_bytes'] / self.stats['total_messages']
                print_colored(f"   Tamaño promedio mensaje: {avg_msg_size:.2f} bytes", 'WHITE')
...
def main():
    ...
    """
    print_colored("\n" + "="*70, 'CYAN')
    print_colored("   MONITOR DE RED MULTICAST", 'CYAN')
    print_colored("="*70 + "\n", 'CYAN')
    ...
    # Información
    print_colored("Este monitor capturará y analizará todo el tráfico multicast", 'YELLOW')
    print_colored(f"Escuchando en: {MULTICAST_GROUP}:{PORT}", 'YELLOW')
    print_colored("\nEl monitor mostrará:", 'WHITE')
    print_colored("  • Estadísticas en tiempo real", 'WHITE')
    print_colored("  • Nodos activos", 'WHITE')
    print_colored("  • Tipos de mensajes", 'WHITE')
    print_colored("  • Tasas de transferencia", 'WHITE')
    print_colored("  • Gráficos de actividad\n", 'WHITE')
